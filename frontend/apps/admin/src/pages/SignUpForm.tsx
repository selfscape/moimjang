...
const AdminSignUpForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      ...
      if (!valid) {
        setErrors((prev) => ({
          ...prev,
          username: "영어 알파벳(A–Z, a–z)만 입력 가능합니다.",
        }));
      } else {
        setErrors((prev) => {
          const { username, ...rest } = prev;
  ...
  const validateStep = () => {
    ...
    if (currentStep === 0) {
      ...
      if (!username) {
        ...
        newErrors.username = "영어 알파벳(A–Z, a–z)만 입력 가능합니다.";
      }
  ...
  return (
    <>
      <Container>
        <CardContainer>
          <FormTitle>어드민 계정 만들기</FormTitle>
          <Form onSubmit={handleSubmit}>
            ...
            <ButtonContainer>
              ...
              <PrimaryButton
                ...
                disabled={
                  currentStep === 1 &&
                  (!formData.username.trim() || !isUsernameValid)
                }
              >
